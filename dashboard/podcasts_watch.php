          ...
          <div class="row">
            <div class="col-md-12 grid-margin stretch-card">
              <div class="card">
                <div class="card-body">
                  <h4 class="card-title"> 
                    <a href="podcasts.php" style="text-decoration: none;">BACK</a> | WATCH VIDEO / LISTEN AUDIO
                  </h4>
                  <div class="form-group">
                    ...
                    if (isset($_GET['id'])) {
                      $id = $_GET['id'];
                      $sql = "SELECT * FROM dashboard_podcasts WHERE id='$id'";
                      $result = mysqli_query($conn, $sql);
                      while ($row = mysqli_fetch_assoc($result)) {
                        $id = $row['id'];
                        $video = $row['video'];
                      }
                      echo "<video width = '100%' controls><source src='uploaded/".$video."' type='video/webm'></video>";
                    } 
                    ?>
                  </div>
                </div>
